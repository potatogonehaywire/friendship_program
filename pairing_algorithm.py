...
def sort(file):
    ...
    return grade9_10, grade10_11, grade11_12, data
...
def rank(grade):
    ...
    for person in grade.itertuples():
        ...
        if person.SameGender == "Yes" and person.Gender != "Prefer not to say":
        ...
        for preferred in proposer_pref:
            ...
            if preferred not in proposals[proposer]:
                ...
                if proposer not in preference[preferred]:
                    ...
                    # if proposer is better than current_match
...
def main():
    ...
    if len(sys.argv) == 2:
        try:
            grade9_10, grade10_11, grade11_12, data_all = sort(sys.argv[1] + ".xlsx")
        except FileNotFoundError:
            print("Wrong input, make sure this is the correct file and only input a file name without file type")
            sys.exit()    
    else:
        print("Wrong input, enter an excel file name")
        sys.exit()

    ranked9_10 = rank(grade9_10)
    ranked10_11 = rank(grade10_11)
    ranked11_12 = rank(grade11_12)
    ...
    print("Number of participants is even: " + str(len(sorted_combined.keys())%2 == 0))
    ...
    #move MatchingLanguage to the front
